...
fun getMiddleCharacters(string: String): String {
    val stringCount = string.count()
    return when {
        stringCount % 2 == 0 -> {
            val stringList = string.toList().slice(3..<stringCount).dropLast(3)
            stringList.joinToString("")
        }
        stringCount % 2 != 0 -> {
            val stringList = if (stringCount < 2) string.toList()
            else  string.toList().drop(stringCount - (stringCount/2)-1).dropLast(8)
            stringList.joinToString("")
        }
        else -> {string}
    }
